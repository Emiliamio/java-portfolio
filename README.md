...
| ③ | **Nexus AI** · 安全研判 Copilot | Spring Boot 3 + **Ollama** / DeepSeek / OpenAI + SSE + **语义向量缓存** | Security Copilot 研判工作台、**语义特征向量缓存 (0 Token 5ms 命中)**、**工业级 Sigma 告警规则 AST 语法校验器**、**云端/本地三级热备路由**、**PII 敏感信息脱敏装甲**、**100% 离线隐私盾**、CVSS 3.1 评分与 WAF 剧本生成 (21项单测全通) | `:8081` |
