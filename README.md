...
| ① | **AuditVault** · 日志审计中台 | Spring Boot 3 + MySQL 8 + Redis 7 + **Kafka KRaft** + **ClickHouse 24.3** + **WebSocket** + **Flyway** + **K8s Helm** | Datadog/SigNoz 级 SOC 遥测大屏、**Flyway 增量版本迁移**、**@AuditLog 无侵入 AOP 埋点**、**Kafka 分布式削峰**、**ClickHouse 45x 毫秒直方图**、**SXSSF 磁盘防 OOM**、分布式 MDC TraceId 追踪、**WebSocket 实时高危推流**、**K8s Helm Chart 弹性扩缩容** (54项单测全通) | `:8080` |
